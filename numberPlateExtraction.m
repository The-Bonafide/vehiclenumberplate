...
[FileName] = uigetfile('*.jpg','Select the Image File for Detecting Number Plate');
if ~(FileName == 0)
    f=imread(FileName); % Reading the number plate image.
    imshow(f);      % For Displaying image to verify the output
    ...
    % Selecting all the bounding boxes in matrix of order numberofboxes;
    ...
    if ~isempty(r) % If succesfully indices of desired boxes are achieved.
        ...
        for v=1:length(r)
            ...
            while letter=='O' || letter=='0' % Since it wouldn't be easy to distinguish
                if v<=2                      % between '0' and 'O' during the extraction of character
                    letter='O';              % in binary image. Using the characteristic of plates in India
        ...
        h = msgbox(noPlate,'Number Plate Extracted');
    ...
    h = msgbox('!! Image not Selected !!', 'ERROR');
end
