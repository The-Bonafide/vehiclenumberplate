...
%   order numberofregions. Numberofregions are the total number of
...
ind=find(Q==10); % Find indices from Q corresponding to frequency '10'.
...
[E,R]=hist(NR2(:,5),20);
Y=find(E==10);                 % Searching for ten characters.
if length(ind)==1 % If ten boxes of interest are succesfully found record
...
elseif isempty(ind) || length(ind)>1 % If there is no vlaue of '10' in the Q vector.
    ...
    if length(ind2)==1
        ...
        container=guesstheten(A,B,(B(2)-B(1))); % Call of function guessthesix.
        if ~isempty(container) % If guesstheten works succesfully.
            r=takeboxes(NR,container,1); % Call the function takeboxes.
        elseif isempty(container)
            container2=guesstheten(E,R,(R(2)-R(1)));
